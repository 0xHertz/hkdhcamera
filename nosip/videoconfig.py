...
class HikVersionAdapter:
    ...
    CONFIG_PROFILES = {
        ('5.0', '6.0'): {
            ...
            'video_path': '/ISAPI/Streaming/channels',
...
class VideoEncodingConfigurator:
    ...
    def start_execution(self):
        ...
        if params := self.validate_inputs():
            ...
            threading.Thread(
                target=self.configure_video_config,
                args=(params,),
                daemon=True
            ).start()

    def configure_video_config(self, params):
        """设备配置主逻辑"""
        try:
            success_entries = []
            failure_entries = []

            for idx, lxj_ip in enumerate(params['lxj_ips']):
                ...
                if not version_info or version_info['device'] == '未知':
                    failure_entries.append({'ip': lxj_ip, 'reason': '无法获取设备版本信息'})
                ...
                # 配置网络设置，发送配置请求
                response = self.send_video_config(
                    lxj_ip=lxj_ip,
                    url=f"http://{lxj_ip}{profile['video_path']}",
                    auth=auth,
                    profile=profile
                )
                if response and response.status_code == 200:
                    success_entries.append({'ip': lxj_ip, 'device': version_info['device'], 'firmware': version_info['firmware']})
                else:
                    failure_entries.append({'ip': lxj_ip, 'reason': f"配置失败，状态码: {response.status_code if response else '无响应'}"})
            ...
            for entry in success_entries:
                self.log(f"成功: IP={entry['ip']}, 设备={entry['device']}, 固件版本={entry['firmware']}")
    ...
    def send_video_config(self, lxj_ip, url, auth, profile):
        """配置平台接入 (GB28181)"""
        try:
            self.log(f"配置设备 {lxj_ip} 的视频编码...")
            self.log(f"获取设备 {lxj_ip} 的现有视频编码配置...")
            response = requests.get(
                url,
            ...
            for channel in existing_config.findall('.//ns:StreamingChannel', ns):
                channel_id = channel.find('ns:id', ns).text
                code_type = channel.find('ns:Video/ns:videoCodecType', ns)
                bit_rate = channel.find('ns:Video/ns:constantBitRate', ns)
                bit_type = channel.find('ns:Video/ns:videoQualityControlType', ns)
                smart_code = channel.find('ns:Video/ns:SmartCodec/ns:enabled', ns)
            ...
            if response and response.status_code == 200:
                self.log(f"✓ 成功配置编码: {lxj_ip}")
            else:
                self.log(f"✗ 配置编码失败: {lxj_ip}, 状态码: {response.status_code}")
                self.log(f"✗ 配置编码失败: {lxj_ip}, 信息: {response.text}")
            return response
        except Exception as e:
            self.log(f"配置编码异常: {traceback.format_exc()}")
...
if __name__ == "__main__":
    root = tk.Tk()
    app = VideoEncodingConfigurator(root)
    root.mainloop()
