...
class HikvisionIPConfigurator:
    def __init__(self, parent=None):
        ...
        self.sip_config_var = tk.BooleanVar(value=False)  # Variable for SIP configuration checkbox
        self.create_widgets()
        self.parent.after(100, self.process_log_queue)

    def toggle_sip_config(self):
        """Toggle SIP configuration based on user input."""
        self.enable_sip_config = self.sip_config_var.get()
    ...
    def create_widgets(self):
        ...
        ttk.Checkbutton(btn_frame, text='启用SIP配置', variable=self.sip_config_var, command=self.toggle_sip_config).pack(side=tk.LEFT, padx=10)
    ...
    def configure_devices(self, params):
        ...
        try:
            for idx, (old_ip, new_ip, new_device_id) in enumerate(zip(params['old_ips'], params['new_ips'], params['new_device_ids'])):
                ...
                # 配置网络设置，发送配置请求
                network_ok = self.send_config_request(
                    old_ip=old_ip,
                    new_ip=new_ip,
                ...
                if self.reboot_var.get() and not self.enable_sip_config and network_ok:
    ...
    def configure_platform_access(self, old_ip, new_ip, url, new_device_id, auth, profile, params, network_ok):
        ...
        try:
            ...
            if response.status_code == 200:
                self.log(f"✓ 成功配置平台接入: {old_ip}->{new_ip}")
                if self.reboot_var.get() and network_ok:
                    self.reboot_device(old_ip, auth, profile)
            else:
